...
public class MemoryMonitor : MonoBehaviour
{
    ...
    private const float BYTES_TO_MB = 1024f * 1024f;
    private static readonly Color32 backgroundColor = new(0, 0, 0, 255);
    
    CircularBuffer<long> allocated, reserved, mono, gcAlloc;
    private CircularBuffer<bool> gcEvents;
    Texture2D graphTexture;
    private Color32[] piexls;

    private Recorder rec;
    private int lastGCCount;
    private long lastMemUsed, maxResrved;
    ...
    void Start()
    {
        ...
        piexls = new Color32[graphTexture.width * graphTexture.height];
    ...
    void Update()
    {
        ...
        UpdateText(gcCountText,gcCount/BYTES_TO_MB,gcHappened?Color.red : gcEventColor);
        
        if(resBytes > maxResrved) maxResrved = resBytes;
    ...
    void DrawGraph()
    {
        ...
        Array.Fill(piexls,backgroundColor);
        ...
        {
            int x = i;
            float scale = graphHeight/(float)Math.Min(maxResrved,1);
