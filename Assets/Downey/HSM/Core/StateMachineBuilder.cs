...
namespace Downey.HSM
{
    public class StateMachineBuilder
    {
        ...
        void Wire(State s, StateMachine m, HashSet<State> visited)
        {
            ...
            var flags = BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy|BindingFlags.Public;
            ...
            foreach (var fld in s.GetType().GetFields(flags))
            {
                ...
                if(!ReferenceEquals(child.Parent,s)) continue;//Ensure its actually our direct child
